...
public interface QuotationRepository extends JpaRepository<Quotation, Long>, QuerydslPredicateExecutor<Quotation> {
    @Query(value = "select q, qm, count(a) from Quotation q left join QuotationMtrl qm on qm.quotation = q left join Agreement a on a.quotation = q where q.contractor.corno = :corno group by q", countQuery = "select count(q) from Quotation q")
