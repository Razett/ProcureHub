...
public interface MaterialRepository extends JpaRepository<Material, Long>, QuerydslPredicateExecutor<Material>, MaterialSearchRepository {
    ...
    @Query("SELECT m FROM Material m JOIN FETCH m.materialGroup JOIN FETCH m.materialWarehouse WHERE m.name LIKE %:name%")
    List<Material> findByMtrlContaining(@Param("name") String name);
}
